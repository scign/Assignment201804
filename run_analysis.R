...
download.file(fileurl, zf)

# first column is ids so just take the second column
names <- read.table(unz(zf, "UCI HAR Dataset/features.txt"), stringsAsFactors=FALSE)[,2]
# replace brackets, dashes and commas with empty string
names <- gsub("[\\(\\)-,]", "", names)

cat("Reading training data\n")
data_train <- read.table(unz(zf, "UCI HAR Dataset/train/X_train.txt"), header=FALSE, col.names=names, stringsAsFactors=FALSE)
subjects_train <- read.table(unz(zf, "UCI HAR Dataset/train/subject_train.txt"), header=FALSE, col.names="subjectid", stringsAsFactors=FALSE)
activities_train <- read.table(unz(zf, "UCI HAR Dataset/train/y_train.txt"), header=FALSE, col.names="activityid", stringsAsFactors=FALSE)
df_train <- cbind(subjects_train, activities_train, data_train)

cat("Reading testing data\n")
data_test <- read.table(unz(zf, "UCI HAR Dataset/test/X_test.txt"), header=FALSE, col.names=names, stringsAsFactors=FALSE)
subjects_test <- read.table(unz(zf, "UCI HAR Dataset/test/subject_test.txt"), header=FALSE, col.names="subjectid", stringsAsFactors=FALSE)
activities_test <- read.table(unz(zf, "UCI HAR Dataset/test/y_test.txt"), header=FALSE, col.names="activityid", stringsAsFactors=FALSE)
df_test <- cbind(subjects_test, activities_test, data_test)

df <- rbind(df_train, df_test)

means.and.stds <- grepl("subjectid|activity|\\.mean\\.|\\.std\\.", names(df))
df2 <- df[, means.and.stds]

cat("Merging with activity labels\n")
activities <- read.table(unz(zf, "UCI HAR Dataset/activity_labels.txt"), col.names=c("activityid","activity"), stringsAsFactors=FALSE)
df3 <- merge(df2, activities)

names(df3) <- c("subjectid",
"activityid",
